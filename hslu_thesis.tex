...
Human-driven and autonomous vehicles will share public roads for decades before full autonomy arrives on scale. Vehicle platooning is a coordinated driving formation in which multiple vehicles travel in close succession under automated spacing control. It offers one of the clearest near-term benefits of this mixed-traffic environment: reduced aerodynamic drag for following vehicles and higher lane throughput per kilometer of road. The difficulty is in entering. A human-driven vehicle that wishes to join an operating platoon must reconcile its driver's intentions with the platoon's automated spacing logic. This thesis develops a game-theoretic shared-control framework, grounded in Nash equilibrium theory, for exactly that problem.


\subsection{Vehicle Platooning}

Vehicle platooning is a driving formation in which multiple vehicles travel closely together at highway speeds, maintaining safe inter-vehicle distances through coordinated control and V2V communication \cite{treiberTrafficFlowDynamics2013}. Drafting reduces fuel consumption by 10--15\% for following vehicles, and smaller following distances allow more vehicles per kilometre of road. Coordinated braking reduces reaction times and lowers collision risk.

Cooperative Adaptive Cruise Control (CACC) is the enabling technology \cite{rajamani_vehicle_2005}. CACC systems exchange speed, acceleration, and braking intentions over Vehicle-to-Vehicle (V2V) links, allowing vehicles to respond faster than human reaction times permit. The desired inter-vehicle gap follows the Constant Time Gap (CTG) policy:
\begin{equation}
d_{\text{des}} = d_0 + h \cdot v
\label{eq:ctg_spacing_v2}
\end{equation}
where $d_0 = 2$\,m is the standstill distance, $h = 1.5$\,s is the time headway, and $v$ is the current vehicle speed. The gap therefore grows linearly with speed, providing appropriate safety margins at all operating conditions.

Most CACC systems assume all platoon members are fully autonomous. This assumption breaks down in mixed traffic, where a human-driven vehicle must negotiate entry into an operating platoon. Safe merging requires explicit handling of both the human driver's intent and the platoon's automated spacing logic, a problem that neither conventional CACC nor standard autonomous control addresses directly.
...
\label{eq:T_lc_min}
\end{equation}
where $\Delta y$ is the lane change distance, $v_x$ is the longitudinal velocity, and the factor 1.875 comes from the maximum of the quintic polynomial's derivative. This constraint ensures the heading angle stays within comfortable limits throughout the maneuver.

Empirical studies provide guidance for selecting appropriate lane change durations. Toledo and Zohar~\cite{toledoModelingDurationLane2007} analyzed naturalistic driving data and found that lane change durations for passenger cars average 5--6~seconds, with significant variation depending on traffic conditions and driver aggressiveness. Lee et al.~\cite{leeComprehensiveExaminationNaturalistic2004a} conducted the largest known naturalistic lane change study (8,667 observed lane changes) and reported durations ranging from approximately 3~seconds for urgent maneuvers to over 8~seconds for cautious ones. These empirical findings inform the lane change duration parameters used in this thesis: human driver durations range from 3.0~s (aggressive) to 6.0~s (cautious), consistent with the naturalistic data, while the autonomous system uses a 1.5$\times$ multiplier yielding durations of 4.5~s to 9.0~s to ensure additional smoothness.
...
The fourth contribution is a \textbf{Bidirectional Longitudinal Safety Field}. We extend the safety field concept to handle risks from both directions during platoon merging. This includes leader force (repulsive force preventing collision with vehicle ahead), follower force (weighted force maintaining safe distance from vehicle behind), asymmetric weighting (follower weight $w_f \in [0.2, 0.5]$ reflects forward-dominant CACC information flow), and dynamic adaptation (parameters adjust based on TTC, gap size, and platoon context).

The fifth contribution is \textbf{Lateral Control with Stanley-Based Human Model}. We adapt the Nash framework to lateral control using a dual-model bicycle architecture: a body-frame plant model for true state propagation and an error-frame controller model for Nash solver feedback, both with state $[y, \dot{y}, \psi, \dot{\psi}]$ and zero-order hold discretization. We employ a modified Stanley controller (linear PD formulation) for human-driver modeling with personality-dependent gains. Different reference trajectories are employed: a 5th-order polynomial for the system and a 3rd-order polynomial for the human. The lane change duration is computed dynamically from the heading angle constraint, with the system duration set to 1.5 times the human duration. Typical values range from 4.5 to 9.0~s for the system and 3.0 to 6.0~s for the human, depending on driver personality type, consistent with empirical measurements of naturalistic lane changes~\cite{toledoModelingDurationLane2007, leeComprehensiveExaminationNaturalistic2004a}. Heading priority ($Q_\psi \gg Q_y$) ensures stability by controlling heading angle first.

The sixth contribution is a \textbf{Unified Control Architecture}. We design both longitudinal and lateral controllers with consistent structure using the same Nash/DMPC framework with DPP-compliant solvers, the same phase detection mechanism (MERGING to FOLLOWING), the same authority allocation based on safety field force, the same soft transition functions for smooth mode changes, and a multi-rate architecture (100~Hz dynamics simulation, 10~Hz longitudinal Nash, 20~Hz lateral Nash).
