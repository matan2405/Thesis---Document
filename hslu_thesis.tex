...
The lateral model employs a bicycle model representation that captures the vehicle's response to steering inputs. This model accounts for tire slip angles and vehicle inertia, which are important for accurate lane-change trajectory tracking. We adopt the \textbf{global-frame} formulation of the bicycle model, in which the lateral position state $y$ represents the vehicle's absolute lateral displacement in the road coordinate system rather than a body-frame lateral velocity. This formulation includes a kinematic coupling term ($v_x \psi$) that directly relates heading deviations to lateral drift, making it particularly suitable for lane-change control where global lateral position is the primary quantity of interest.
...
\end{equation}
where $a_{\min} = -3.5$ m/s² (firm braking) and $a_{\max} = 2.5$ m/s² (moderate acceleration). The braking limit of $-3.5$ m/s² provides margin above emergency braking capability while remaining within the comfort range for assisted driving.
...
\label{eq:lat_state}
\end{equation}
where $y$ is the lateral position in the global road coordinate frame in meters, $\dot{y}$ is the lateral velocity in meters per second, $\psi$ is the heading angle (yaw angle relative to the road) in radians, and $\dot{\psi}$ is the yaw rate in radians per second.

\subsection{Global-Frame Bicycle Model}
\label{sec:global_frame_model}

The standard bicycle model \cite{rajamani_vehicle_2005} describes the lateral dynamics in the body-fixed frame. For lane-change control, however, it is more natural to track the vehicle's absolute lateral position in the road coordinate system. We therefore adopt the \textbf{global-frame formulation}, in which the first state equation includes a kinematic coupling term that converts heading deviations into lateral drift.

The equations of motion for the bicycle model are derived from Newton's second law for lateral translation and rotation \cite{rajamani_vehicle_2005, hoffmannAutonomousAutomobileTrajectory2007}:
\begin{align}
m \ddot{y}_{\text{body}} &= F_{yf} + F_{yr} \label{eq:lat_force} \\
I_z \ddot{\psi} &= l_f F_{yf} - l_r F_{yr} \label{eq:yaw_moment}
\end{align}
where $m$ is the vehicle mass, $I_z$ is the yaw moment of inertia, $F_{yf}$ and $F_{yr}$ are the lateral tire forces at the front and rear axles, $l_f$ is the distance from the center of gravity (CG) to the front axle, and $l_r$ is the distance from the CG to the rear axle.

For small slip angles, the lateral tire forces are proportional to the slip angles. Here the cornering stiffness coefficients $C_f$ and $C_r$ represent the \textbf{per-tire} stiffness, so that the total axle force requires a factor of two to account for both left and right tires:
\begin{align}
F_{yf} &= -2 C_f \alpha_f \label{eq:front_tire} \\
F_{yr} &= -2 C_r \alpha_r \label{eq:rear_tire}
\end{align}

The slip angles depend on the vehicle's motion and the steering angle:
\begin{align}
\alpha_f &= \frac{\dot{y}_{\text{body}} + l_f \dot{\psi}}{v_x} - \delta \label{eq:front_slip} \\
\alpha_r &= \frac{\dot{y}_{\text{body}} - l_r \dot{\psi}}{v_x} \label{eq:rear_slip}
\end{align}
where $\delta$ is the front wheel steering angle and $v_x$ is the longitudinal velocity.

The global lateral position $y$ is related to the body-frame lateral velocity by:
\begin{equation}
\dot{y} = \dot{y}_{\text{body}} + v_x \sin\psi \approx \dot{y}_{\text{body}} + v_x \psi
\label{eq:global_y}
\end{equation}
where the small-angle approximation $\sin\psi \approx \psi$ is valid for the heading angles encountered during controlled lane changes (typically $|\psi| < 2^\circ$).

Substituting the tire forces, slip angles, and the kinematic coupling into the state equations, the continuous-time state-space representation becomes:
\begin{equation}
...
A_c = \begin{bmatrix}
0 & 1 & v_x & 0 \\[6pt]
0 & -\dfrac{2(C_f + C_r)}{m v_x} & 0 & -v_x - \dfrac{2(l_f C_f - l_r C_r)}{m v_x} \\[6pt]
0 & 0 & 0 & 1 \\[6pt]
...
The input matrix is:
\begin{equation}
B_c = \begin{bmatrix} 0 \\[4pt] \dfrac{2 C_f}{m} \\[4pt] 0 \\[4pt] \dfrac{2 l_f C_f}{I_z} \end{bmatrix}
\label{eq:Bc_lat}
\end{equation}

\paragraph{Comparison with the body-frame formulation.}
In the classical body-frame bicycle model \cite{rajamani_vehicle_2005}, the first state equation is $\dot{y}_{\text{body}} = \dot{y}_{\text{body}}$ (an identity), and the $A_c(1,3) = 0$ element reflects that heading does not directly enter the body-frame lateral velocity equation. In the global-frame formulation used here, $A_c(1,3) = v_x$ captures the kinematic coupling between heading and global lateral position. This means a small heading error $\psi$ produces a lateral drift rate of $v_x \psi$, which is precisely the physical behavior that must be tracked during lane changes. The $A_c(2,3) = 0$ entry (compared to $(C_f+C_r)/m$ in the body-frame version) results from the coordinate transformation and the use of per-tire cornering stiffness with the explicit factor of two.

The state matrix $A_c$ depends on the longitudinal velocity $v_x$. This means that the lateral model is a linear parameter-varying (LPV) system. For the MPC implementation, we update the matrices at each control step using the current velocity.
...
The bicycle model parameters are based on an Audi TT Coupé 2.0 TFSI (230~PS), chosen as a representative compact sports sedan \cite{rajamani_vehicle_2005, fernandez_vehicle_nodate}. Table~\ref{tab:vehicle_params_ch2} summarizes the parameters used in the simulation.
...
Yaw moment of inertia & $I_z$ & 2500 kg$\cdot$m² \\
Wheelbase & $L$ & 2.505 m \\
...
Maximum steering rate & $\dot{\delta}_{\max}$ & $15^\circ$/ss (0.262 rad/s) \\
Nominal velocity & $v_x$ & 20 m/s (72 km/h) \\
\hline
\end{tabular}
\caption{Vehicle parameters based on the Audi TT Coupé 2.0 TFSI}
\label{tab:vehicle_params_ch2}
\end{table}

The symmetric CG position ($l_f = l_r = L/2$) produces neutral steer characteristics, which simplifies the dynamics (several $l_f C_f - l_r C_r$ terms vanish when $l_f = l_r$ and $C_f = C_r$). The cornering stiffness values of 55,000~N/rad per tire were selected through simulation-based tuning to produce realistic lateral response at the nominal velocity of 20~m/s. These values yield a total axle stiffness of 110,000~N/rad (with the factor of two in the equations of motion), which is consistent with standard passenger vehicles in the 1300~kg class~\cite{rajamani_vehicle_2005}.

\subsection{Sampling Rate Selection}

The vehicle dynamics simulation runs at 100~Hz ($\Delta t_{\text{sim}} = 0.01$~s), which provides high-fidelity integration of the bicycle model. The lateral Nash MPC controller operates at a lower rate of 20~Hz ($\Delta t_{\text{Nash}} = 0.05$~s), meaning the optimization is solved every fifth simulation step. Between Nash updates, the most recent steering command is held constant (zero-order hold).

The 20~Hz Nash rate was chosen as a compromise between computational cost and control bandwidth. At the nominal velocity of 20~m/s, the bicycle model's dominant yaw time constant is approximately 0.15--0.3~s, and the 20~Hz rate provides 3--6 control updates per time constant, which is sufficient for accurate trajectory tracking.

\subsection{Dual-Model Architecture}
\label{sec:dual_model}

The lateral control implementation employs a dual-model architecture in which two versions of the bicycle model run in parallel at each simulation time step. The first model, referred to as the \textbf{plant model}, uses the standard body-frame formulation. The second model, referred to as the \textbf{error model}, uses a modified formulation better suited for MPC prediction.

The plant model uses the standard body-frame state matrix, where the first-row coupling element is zero ($A_c^{\text{body}}(1,3) = 0$), and the lateral velocity equation follows the classical derivation of Rajamani~\cite{rajamani_vehicle_2005}:
\begin{equation}
A_c^{\text{body}} = \begin{bmatrix}
0 & 1 & 0 & 0 \\[6pt]
0 & -\dfrac{2(C_f + C_r)}{m v_x} & 0 & -v_x - \dfrac{2(l_f C_f - l_r C_r)}{m v_x} \\[6pt]
0 & 0 & 0 & 1 \\[6pt]
0 & -\dfrac{2(l_f C_f - l_r C_r)}{I_z v_x} & 0 & -\dfrac{2(l_f^2 C_f + l_r^2 C_r)}{I_z v_x}
...
The error model modifies the second and fourth rows to express the dynamics in terms of force coupling rather than kinematic coupling. For a straight reference road, the error model state matrix is:
\begin{equation}
A_c^{\text{error}} = \begin{bmatrix}
0 & 1 & 0 & 0 \\[6pt]
0 & -\dfrac{2(C_f + C_r)}{m v_x} & \dfrac{2(C_f + C_r)}{m} & -\dfrac{2(l_f C_f - l_r C_r)}{m v_x} \\[6pt]
0 & 0 & 0 & 1 \\[6pt]
0 & -\dfrac{2(l_f C_f - l_r C_r)}{I_z v_x} & \dfrac{2(l_f C_f - l_r C_r)}{I_z} & -\dfrac{2(l_f^2 C_f + l_r^2 C_r)}{I_z v_x}
\end{bmatrix}
\label{eq:Ac_error}
\end{equation}

The key differences are: $A_c^{\text{error}}(2,3) = 2(C_f+C_r)/m$ (compared to $0$ in the body-frame model) and $A_c^{\text{error}}(4,3) = 2(l_f C_f - l_r C_r)/I_z$ (compared to $0$ in the body-frame model). The input matrix $B_c$ and the output matrix $C$ are identical for both models.

The error-model formulation follows the approach of Rajamani~\cite{rajamani_vehicle_2005}, Section~2.5, where the lateral error dynamics are expressed relative to a reference path. For a straight reference road (zero curvature), the error model captures how tire forces restore the vehicle toward the reference trajectory. This formulation is better suited for the MPC controller because it expresses the restoring dynamics through explicit force-coupling terms in the state matrix, providing more consistent linearization across the prediction horizon. With the symmetric CG position ($l_f = l_r$, $C_f = C_r$), the term $l_f C_f - l_r C_r$ vanishes, and the error model simplifies so that $A_c^{\text{error}}(4,3) = 0$, while $A_c^{\text{error}}(2,3) = 2(C_f+C_r)/m$ remains the active coupling term.

Both models share the same input matrix $B_c$ and are discretized at the simulation rate (100~Hz) using the ZOH method (Equation~\eqref{eq:augmented}). At each time step, both models receive the same rate-limited steering command $\delta[k]$. The plant model state is used for simulation output and safety field evaluation, while the error model state is provided to the Nash equilibrium solver for state feedback (via the \texttt{get\_state\_vector()} interface). This separation ensures that the controller operates on a representation optimized for prediction accuracy, while the simulation faithfully represents the true vehicle response.

\section{Multi-Rate Control Architecture}

The longitudinal and lateral controllers operate at different rates within a three-level timing hierarchy:
...
Lateral Nash MPC & 20 Hz & 0.05 s & Steering optimization \\
Longitudinal Nash MPC & 10 Hz & 0.10 s & Acceleration optimization \\
\hline
\end{tabular}
...
Both controllers run in parallel. The longitudinal controller updates every 100~ms to compute a new acceleration command. The lateral controller updates every 50~ms to compute a new steering command. Between control updates, the respective commands are held constant.

The controllers share state information through the vehicle state vector, which is updated at the fastest rate (100~Hz). This ensures that both controllers have access to the most recent position, velocity, and heading information.

The multi-rate architecture has several advantages. It allows each controller to operate at a rate appropriate for its dynamics. It reduces computational load by avoiding unnecessary high-rate longitudinal optimization. It aligns with the typical architecture of production vehicle control systems, in which different subsystems operate at different rates.
