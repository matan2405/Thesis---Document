...
\label{eq:T_lc_min}
\end{equation}
where $\Delta y$ is the lane change distance, $v_x$ is the longitudinal velocity, and the factor 1.875 comes from the maximum of the quintic polynomial's derivative. This constraint ensures the heading angle stays within comfortable limits (typically $\psi_{\max} < \SI{2}{\degree}$) throughout the maneuver.
...
The fifth contribution is \textbf{Lateral Control with Stanley-Based Human Model}. We adapt the Nash framework to lateral control using a 4-state global-frame bicycle model with states $[y, \dot{y}, \psi, \dot{\psi}]$ and proper zero-order hold discretization. We employ a modified Stanley controller (linear PD formulation) for human-driver modeling with personality-dependent gains. Different reference trajectories are employed: a 5th-order polynomial for the system (14 seconds base duration) and a 3rd-order polynomial for the human (10--14 seconds depending on driver type). Heading priority ($Q_\psi \gg Q_y$) ensures stability by controlling heading angle first.
...
\label{eq:global_y}
\end{equation}
where the small-angle approximation $\sin\psi \approx \psi$ is valid for the heading angles encountered during controlled lane changes (typically $|\psi| < 2°$).
...
Front cornering stiffness (per tire) & $C_f$ & 80,000 N/rad \\
Rear cornering stiffness (per tire) & $C_r$ & 80,000 N/rad \\
Overall length & --- & 4.177 m \\
Overall width & --- & 1.832 m \\
Maximum steering angle & $\delta_{\max}$ & $25°$ (0.436 rad) \\
Maximum steering rate & $\dot{\delta}_{\max}$ & $15°$/s (0.262 rad/s) \\
...
The symmetric CG position ($l_f = l_r = L/2$) produces neutral steer characteristics, which simplifies the dynamics (several $l_f C_f - l_r C_r$ terms vanish when $l_f = l_r$ and $C_f = C_r$). The cornering stiffness values of 80,000~N/rad per tire are representative of high-performance road tires on a vehicle of this class.
...
The steering output is saturated at the vehicle's physical steering limit ($\pm 25°$) and low-pass filtered ($\alpha = 0.2$) to prevent abrupt corrections.
...
The small magnitude of $k_e$ (order $10^{-3}$) reflects the fact that steering angles are measured in radians while lateral errors are measured in meters. At a typical cross-track error of 1~m, the normal driver produces a steering correction of $-0.005 \times 1.0 = -0.005$~rad ($\approx 0.3°$), which is a realistic, gentle correction consistent with human driving behavior at highway speeds. The heading gain $k_\psi$ is three orders of magnitude larger, reflecting the importance of heading alignment: a heading error of $1°$ ($\approx 0.017$~rad) produces a steering correction of $-0.5 \times 0.017 = -0.009$~rad ($\approx 0.5°$).
...
\begin{equation}
-0.436 \leq \delta_i \leq 0.436 \text{ rad} \quad (\approx \pm 25°)
\label{eq:lat_bounds}
\end{equation}
...
\label{eq:lat_rate}
\end{equation}
with $\dot{\delta}_{\max} = 0.262$~rad/s ($15°$/s). This value corresponds to the Audi TT steering system specification and ensures smooth steering transitions consistent with passenger comfort expectations.
...
With $\psi_{\max} = \SI{1.82}{\degree}$ (providing margin below the $\SI{2}{\degree}$ requirement), $|\Delta y| = 3.5$~m (one lane width), and $v_x = 20$~m/s, this yields $T_{\text{lc}} \geq 10.4$~s. The implementation uses $T_{\text{lc}} = 14$~s as the base duration, which is dynamically increased if the heading constraint requires it.
...
\hline
Input bounds & $[-3.5, 2.5]$ m/s$^2$ & $\pm 0.436$ rad ($\pm 25°$) \\
