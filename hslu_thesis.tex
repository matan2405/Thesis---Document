...
\end{equation}
where $u_1^{*}$ is the optimal control of the autonomous system, and $u_2^{*}$ is the
optimal control of the human driver at the current time step. The two controls are
summed directly rather than blended by a convex combination, since the authority balance
is encoded in the Nash cost function of the human player rather than in an outer weighting
rule \cite{pustilnikGeneralizedNashEquilibrium2025}.

The authority balance is governed by an authority weight
\begin{equation}
\alpha = \frac{1}{1 + \lambda}
\label{eq:alpha_lambda}
\end{equation}
where $\alpha \in (0, 1]$ scales the control-effort weight of the human player, and
$\lambda \geq 0$ is the authority ratio derived from the safety field force. The
human's control-effort matrix is set to $R_2(\lambda) = R_2^{0} / \alpha$, so that a
small $\alpha$ (equivalently, a large $\lambda$) penalizes deviations of $u_2^{*}$
from zero and drives the human input toward the null control. Under nominal risk
($\lambda = 0$) the weight reduces to $\alpha = 1$ and the human retains full
authority. Under maximal risk ($\lambda \to \infty$) the weight decays to
$\alpha \to 0$ and the system control $u_1^{*}$ becomes the dominant contribution to
$u_{\text{shared}}$. The value of $\lambda$ is determined at each control step from
the magnitude of the safety field force computed in the preceding subsection.
...
The fourth contribution is a bidirectional longitudinal safety field. The field
combines an elliptic leader force \cite{liSharedControlNovel2019} with a time-to-collision
(TTC) force floor, a piecewise hard-zone response at close range, and a
follower-weighted rear force. The TTC floor is
\begin{equation}
...
\label{eq:ttc_floor}
\end{equation}
where $F_{\max} = 800$\,N is the saturation force, $\mathrm{TTC}_{\text{warn}} = 8$\,s
is the TTC at which the floor begins to rise, and $\mathrm{TTC}_{\text{crit}} = 1.5$\,s
is the TTC at which the floor reaches saturation. The hard-zone response saturates
the leader force below $7$\,m gap and linearly interpolates between $7$ and $8$\,m.
The follower contribution is weighted by $w_f = 0.5$ at steady state.
...
The Nash solver enforces two levels of constraints. The first level is a wide solver
ceiling imposed as box constraints on the OSQP quadratic program:
\begin{equation}
u \in [-20,\; 10]\ \text{m/s}^2, \qquad
|\Delta u| \leq 15\ \text{m/s}^2\text{/step}
\label{eq:long_solver_box}
\end{equation}
These bounds are deliberately wide: they act as numerical guardrails rather than
comfort limits. The second level consists of physical bounds computed at each tick
from the current vehicle state via \texttt{UpdatePhysicalBounds()}, which clips the
commanded acceleration to what the powertrain and brake system can actually deliver.

For the human player, the inter-step rate cap is tighter:
\begin{equation}
|\Delta u_2| \leq 2.0\ \text{m/s}^2\text{/step}\ \rightarrow\ 0.2\ \text{m/s}^2\text{/s}\ \text{at 10\,Hz}
\label{eq:long_human_jerk}
\end{equation}
This cap prevents inter-step oscillations in $u_2^*$ while preserving the Nash
game's freedom to assign a nonzero human control. Comfort shaping beyond this hard
cap is handled through the $R_2$ weight schedule described in
...
centre of gravity to the front axle, and $l_r$ is the distance from the centre of gravity
to the rear axle.
...
\end{align}
where the factor of two converts per-wheel stiffness to the full axle, and the slip angles are
\begin{align}
...
\end{align}
where $\delta$ is the front wheel steering angle. Substituting into the equations of motion
yields the continuous-time state-space representation
\begin{equation}
\dot{\mathbf{x}} = A_c(v_x)\, \mathbf{x} + B_c\, \delta
\label{eq:lat_ss_continuous}
\end{equation}
where the velocity-dependent state matrix is
\begin{equation}
A_c = \begin{bmatrix}
0 & 1 & 0 & 0 \\[6pt]
...
\label{eq:Bc_lat}
\end{equation}
Because $A_c$ depends on $v_x$, the model is a linear parameter-varying (LPV) system.
The matrices are recomputed, and the OSQP prediction matrices rebuilt, whenever the
longitudinal speed drifts more than $2\;\mathrm{m/s}$ from the last linearisation point.
