...
To model the human driver's longitudinal behavior, we use the Intelligent Driver Model (IDM) developed by Treiber et al. \cite{treiber_congested_2000, treiberTrafficFlowDynamics2013}. The IDM is a car-following model that produces realistic acceleration behavior based on the current velocity, the gap to the leading vehicle, and the relative velocity.
...
\subsection{Lateral: Modified Stanley Controller}
\label{sec:stanley_controller}

To model the human driver's lateral behavior during lane changes, we use a modified version of the Stanley controller \cite{thrun_stanley_2006, abdelmoniem_path-tracking_2020}. The classical Stanley controller combines heading error correction with a nonlinear cross-track error term. In our implementation, we adopt a \textbf{linear proportional-derivative (PD) formulation} that is better suited to integration with the Nash equilibrium framework, where linearity of the human model improves convergence:

\begin{equation}
\delta_{\text{human}} = -k_e \cdot e_y - k_\psi \cdot e_\psi
\label{eq:stanley_modified}
\end{equation}
where $e_y = y - y_{\text{ref}}$ is the lateral position error, $e_\psi = \psi - \psi_{\text{ref}}$ is the heading angle error, $k_e$ is the cross-track gain, and $k_\psi$ is the heading gain.

This formulation preserves the two-term structure of the Stanley controller (position correction plus heading correction) while replacing the nonlinear $\arctan(k_e \cdot e / v_x)$ term with a linear gain. The linear form is appropriate for lane-change control because the errors are small enough that the $\arctan$ function operates in its approximately linear region. The separate heading gain $k_\psi$ provides independent control of heading response, which is important for passenger comfort during lane changes.

The steering output is saturated at the vehicle's physical steering limit ($\pm 25^\circ$) and low-pass filtered ($\alpha = 0.2$) to prevent abrupt corrections.

\subsection{Driver Personality Types}

We model three types of human drivers with different parameter settings, summarized in Table~\ref{tab:driver_personality}.

\begin{table}[ht]
\centering
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Driver Type} & \textbf{IDM $T$ [s]} & \textbf{IDM $b$ [m/s²]} & \textbf{Stanley $k_e$} & \textbf{Stanley $k_\psi$} \\
\hline
Cautious   & 2.0 & 1.5 & 0.003 & 0.3 \\
Normal     & 1.5 & 2.0 & 0.005 & 0.5 \\
Aggressive & 1.0 & 2.5 & 0.008 & 0.7 \\
\hline
\end{tabular}
\caption{Human driver personality parameters}
\label{tab:driver_personality}
\end{table}

The small magnitude of $k_e$ (order $10^{-3}$) reflects the fact that steering angles are measured in radians while lateral errors are measured in meters. At a typical cross-track error of 1~m, the normal driver produces a steering correction of $-0.005 \times 1.0 = -0.005$~rad ($\approx 0.3^\circ$), which is a realistic, gentle correction consistent with human driving behavior at highway speeds. The heading gain $k_\psi$ is three orders of magnitude larger, reflecting the importance of heading alignment: a heading error of $1^\circ$ ($\approx 0.017$~rad) produces a steering correction of $-0.5 \times 0.017 = -0.009$~rad ($\approx 0.5^\circ$).
...
This chapter presented the mathematical models used for vehicle dynamics in this thesis. The longitudinal model is a double integrator with state variables for position and velocity, discretized at 10~Hz using ZOH. The lateral model is a 4-state bicycle model in two formulations: a body-frame plant model for state integration and an error-frame controller model for MPC feedback, both capturing lateral position, lateral velocity, heading, and yaw rate. The body-frame model follows the standard derivation, while the error model includes explicit force-coupling terms ($A_c(2,3) = 2(C_f+C_r)/m$) that improve prediction consistency for the Nash solver. Vehicle dynamics are simulated at 100~Hz, with the lateral Nash MPC operating at 20~Hz and the longitudinal Nash MPC at 10~Hz.
...
Human driver behavior is modeled using the Intelligent Driver Model for longitudinal control and a modified Stanley Controller (linear PD formulation) for lateral control. Three personality types (cautious, normal, and aggressive) are defined through different parameter settings, with the Stanley gains reflecting realistic steering magnitudes at highway speeds.

The multi-rate architecture allows each controller to operate at a rate appropriate for its dynamics while sharing state information through a common vehicle state vector updated at 100~Hz.
